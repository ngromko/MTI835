...
*/
 	void show(const std::string& postscriptFilename,bool bWithConstraints=true);
...
*
*/
 	void show(Visualizer2* pVis,bool bWithConstraints=true);
...
* @param cis
* CIS_CONFORMING_DELAUNAY: Automatically subdivide the constraint edges
* in order to keep the empty circle property (Conforming Delaunay
* triangulation)@n
* CIS_CONSTRAINED_DELAUNAY: Do \e not subdivide the constraint edges.
* Note that already existing elements may enforce edge subdivision
* nevertheless. Subsequent operations (i.e. Delaunay meshing through
* the refine() method) may also split those constraints. @n
*
...
*/
	...
	Zone2* createZone( std::vector<Triangle2*>& vTriangles );
...
* be called again.
*
* Since Fade version 1.33 intersecting constraint segments are
...
* the coordinates must be rounded.
*/
