...
*
* Returns the squared length of the \e ith edge. This method ignores
* the z-coordinate (if any).
*/
...
inline std::ostream &operator<<(std::ostream &stream, const Triangle2& c)
{
	stream << "Triangle(" << &c << "):"<<std::endl;
	for(int ith=0;ith<3;++ith)
	{
		Triangle2* neig=c.getOppositeTriangle(ith);
		stream<<ith<<": "<<*c.getCorner(ith)<<"\t oppTriangle: "<<neig<<std::endl;
	}
	return stream;
}
